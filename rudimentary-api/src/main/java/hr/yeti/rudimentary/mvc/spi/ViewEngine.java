...
/**
 * SPI in charge of processing MVC. It is mandatory to have ViewEngine registered in order for {@link ViewEngine} to work properly. Providers of this SPI transform dynamic HTML templates to a raw
 * string representing HTML to be sent out in a HTTP response.
 *
...
public interface ViewEngine extends Instance {
    ...
    /**
     * A core method of the MVC view engine. This is the method that will call the underlying template engine's method which dynamically applies context values to a template file and produces a HTML
     * string to be sent out in a HTTP response.
     *
     * Some of the template engines available are: Pebble, Jade, Apache Velocity, Apache FreeMarker, Thymeleaf, Mustache, Handlebars etc.
     *
     * This framework comes with out-of-the box implementation of the Pebble templating engine which you can use as an example if you wish to add your own ViewEngine provider. It can be found in
    ...
    /**
     * Set the location of the template directory. Default value is provided via {@link Config} property: {@code mvc.templatesDir=view}
     *
