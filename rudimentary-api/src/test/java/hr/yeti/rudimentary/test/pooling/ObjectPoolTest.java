...
public class ObjectPoolTest {
  ...
  public void test_pool_shutdown() {
    ...
    given:
    pool = new IntegerPool();
    pool.initialize();

    then:
    assertEquals(ObjectPool.PoolStatus.UP, pool.status());
    ...
    then:
    assertEquals(ObjectPool.PoolStatus.DOWN, pool.status());
  }
  ...
  public void test_pool_initial_settings() {
    ...
    expect:
    assertEquals(3, pool.objectCount());
  ...
  public void test_pool_borrow_object_in_FIFO_order() {
    ...
    then:
    assertEquals(0, intObject0);
    assertEquals(1, intObject1);
    assertEquals(2, intObject2);

    and:

    when: // Return objects to pool in reverse order
  ...
  public void test_pool_borrow_object_on_empty_pool() {
    ...
    when: // Try to borrow when pool is empty
    intObject = pool.borrow();
